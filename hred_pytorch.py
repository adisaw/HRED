...
class HRED_QA(object):
    def __init__(self,
        ...
        if self.dictionary and type(self.dictionary) == str and not self.word2id:
            dt = pkl.load(open(self.dictionary,'rb'))
            for d in dt:
            	print(d)
            self.word2id = {k:v for k,v in dt.items()}
            self.id2word = {v:k for k,v in dt.items()}
    ...
    def trainIters(self,encoder=None, decoder=None, 
            context=None, print_every=100, plot_every=100, evaluate_every=500, 
            learning_rate=None):
        ...
        while True:
            ...
            if iter % print_every == 0:
                ...
                f.write(str(iter))
                f.write(',')
                f.write(str(print_loss_avg.item()))
                f.write('\n')
                f.close()
            ...
            for di in range(max_length):
                ...
                if last:
                    if ni == self.EOS_token:
                    	...
                    	decoded_words.append(self.id2word[ni.item()])
